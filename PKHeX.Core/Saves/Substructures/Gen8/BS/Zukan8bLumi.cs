...
public sealed class Zukan8bLumi : ZukanBase<SAV8BSLuminescent>
{
    private const int COUNT_SPECIES = 1010;
    ...
    private const int OFS_FLAG_REGIONAL = OFS_LANGUAGE + SIZE_LANGUAGE;
    private const int OFS_FLAG_NATIONAL = OFS_FLAG_REGIONAL + 4;

    private const int LANGUAGE_NONE = 0;
    private const int LANGUAGE_ALL =
    ...
    public ZukanState8b GetState(ushort species) => (ZukanState8b)(SAV.Data[PokeDex + GetStateStructOffset(species)] >> ((species & 1) * 4) & 0xF);

    public void SetState(ushort species, ZukanState8b state) => SetNibble(ref SAV.Data[PokeDex + GetStateStructOffset(species)], (byte)((species & 1) * 4), (byte)state);

    public bool GetBoolean(int index, int baseOffset) => (SAV.Data[PokeDex + GetBooleanStructOffset(index, baseOffset)] >> (index % 8) & 1) == 1;

    public void SetBoolean(int index, int baseOffset, bool value) => SetBit(ref SAV.Data[PokeDex + GetBooleanStructOffset(index, baseOffset)], (byte)(index % 8), value);
    ...
    public override void SetDex(PKM pk)
    {
        ...
        if (species is 0 or > COUNT_SPECIES)
            return;
    ...
    {
        for (ushort species = 1; species <= COUNT_SPECIES; species++)
    ...
    {
        var pt = Personal;
        for (ushort i = 1; i <= COUNT_SPECIES; i++)
        {
            if (!GetSeen(i))
